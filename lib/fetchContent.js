...
function searchDeeper(params, maxDepth = DEFAULT_MAX_SEARCH_DEPTH){
	...
	.then( searchItem => {
		...
		searchItem.thisNumResults   = (sapiObj.results[0].results) ? sapiObj.results[0].results.length : 0;
...
function v2v1Concordance( v2Url ){
	const url = `https://api.ft.com/concordances?apiKey=${CAPI_KEY}&conceptId=${v2Url}`;
	...
	.then(concordanceData => {
		...
		if ( concordanceData && concordanceData.concordances ){
			...
			concordanceData.concordances.forEach( c => {
				if (
						c.identifier
						&& c.identifier.authority === 'https://api.ft.com/system/FT-TME'
...
function v1IdsOfV2Annotations( v2Annotations ){
	const promises = v2Annotations.map( v1IdsOfV2Annotation );
	return Promise.all( promises )
	.then( v1IdsLists => v1IdsLists.filter( list => list !== null && list.length > 0 ))
	.then( v1IdsLists => [].concat.apply([], v1IdsLists) )
