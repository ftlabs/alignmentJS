...
function v1IdsOfV2Annotations( v2Annotations ){
	const promises = v2Annotations.map( tmeIdsOfV2Annotation );
	return Promise.all( promises )
	.then( tmeIdsLists => tmeIdsLists.filter( list => list !== null && list.length > 0 ))
	.then( tmeIdsLists => [].concat.apply([], tmeIdsLists) )
	.then( tmeIds => [...new Set(tmeIds)] )
	;
}
